...
function checkForm(){
	...
	var fielde = new Array("app_lmt","cust_name","last_name","first_name","nationality","birthday","fami_member","cert_no","pre_addr","pre_city_code","pre_post","pre_zone_no","pre_phone","per_income","ave_income","comp_name","comp_addr","comp_city_code","comp_post","comp_zone_no","comp_phone","work_years","lm_name","lm_pre_zoneno","lm_pre_phone","app_source","exam_node_name"); 	
	...
	var info = new Array("拟申请额度","客户姓名","客户姓(拼音)","客户名(拼音)","客户国籍","客户出生日期","家庭人口","证件号码","现住址","现住址城市代码","现住址邮编","个人年收入","家庭人均年收入","单位全称","单位地址","单位地址城市代码","单位邮编","单位电话区号","单位电话号码","现单位工作年限","联系人姓名","申请表来源","网点编号");  
	...
	var infoB = new Array("客户性别","婚姻状况","证件类型","教育程度","住宅情况","单位性质","行业类别","职称","职务","领卡方式","密码,对帐单邮寄地址","联系人性别","与申请人关系","调查方式"); 
	...
	//检查姓拼音、名拼音长度和 add by lixq 20090811
   ...
   if(getSelectedButton(document.forms[0].cert_type)== 0){
	...
	var info = new Array("申请人单位电话区号","申请人单位电话号码"); 
	...
	//若证件类型为其他军队单位公务卡2009-8-6 modify lixq
	...
	//选择住址情况,若为'自购有贷款房',则"月还款"文本框为必填项
	...
	*/
	...
	var infov = new Array("姓名(中文)","证件号码","现住址","现住址电话号码","单位全称","单位地址","单位电话号码","预留问题","预留答案","联系人姓名(中文)","联系人现住址电话号码","其他证件名称"); 
	...
	var infov = new Array("拟申请额度","个人年收入","家庭人均年收入","房贷月还款"); 
	//检察输入信息是否为空 
	var tmp; 
	for(var i=0;i<fieldv.length;i++)	{ 
	 	tmp ="document.forms[0]."+fieldv[i]+".value"; 
	...
	if(document.forms[0].pre_city_code.value == document.forms[0].comp_city_code.value){
		if((document.forms[0].pre_zone_no.value) != document.forms[0].comp_zone_no.value){
			...
			if(document.forms[0].pre_zone_no.value !=""){
				if(MsgError == ""){
					MsgError = MsgError + "现住址电话区号或单位电话区号错误";}
				else{
					MsgError = MsgError + "\n现住址电话区号或单位电话区号错误";}
			}
		}
		if((document.forms[0].pre_post.value).substring(0,3) != (document.forms[0].comp_post.value).substring(0,3)){
			...
			if(MsgError == ""){
				MsgError = MsgError + "现住址邮编前三位与单位邮编前三位不一致";}
			else{
				MsgError = MsgError + "\n现住址邮编前三位与单位邮编前三位不一致";}
		}
	}
