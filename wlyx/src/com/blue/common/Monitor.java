...
public class Monitor {
	...
	public static Pattern awards = Pattern.compile("awards_view \\( (\\d+) \\)\">(全民福利礼包|辎重营荣誉礼包|中秋.*?月饼礼包|.{2}成绩单|.{2}月饼|竞技礼包|.{2}天机石|英雄纪念礼包|减负报告单|完成任务得礼包|竞技场挑战得礼包|茶油灯芯礼包|兔年元宵灯笼|纸糊帖礼包|竹篾骨架礼包|春节红包|春节黄酒礼包|减负报告单出售|国家贡献\\d+级红包|.{0,10}援军令|青铜宝箱兑换|.{1,2}龙年|.{1,11}集字|勇者闯关得金券|幻境塔通关礼包|遗失的包裹|挑战礼包|上线得礼包|.{1,10}护卫).*?(立即领取|已领取)",Pattern.DOTALL);
	...
	public static final String GET_GUO_DU_AWARD = "modules/warrior.php?act=arena&op=get_prise&team_mode=0&submit=1&arena_key=";
	...
	public static void getGuoDuAward(User user){
		...
		if(Tools.success(page)){
			...
			if(m.find()){
				//http://s4.verycd.9wee.com/modules/warrior.php?act=arena&op=get_prise&submit=1&arena_key=5_1_1331654400&team_mode=0&timeStamp=1331702195358
	...
	public static boolean atFuBen(User user){
		...
		if(m.find()){
			...
			if("函谷关".equals(map) || "秦始皇陵".equals(map) ||"异兽志".equals(map)){
				return true;
			}
		}
		m = p.matcher(page);
	...
	public static boolean dailyWeals(User user){
		String s = user.getDailyWeal();
		if("0".equals(s)){
			return true;
		}
		String url = user.getUrl()+DAILY_WEALS+s+Tools.getRandAndTime().substring(1);
		String page = PageService.getPageWithCookie(url, user);
		if(Tools.success(page)){
