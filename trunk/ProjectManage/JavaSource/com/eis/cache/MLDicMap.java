...
public class MLDicMap {
	...
	public static synchronized void loadDicMap() {
		...
		try {
			...
			ResultSet rs1 =
				db.sqlQuery(
					"select * from  ep_dic_type where DIC_LEVEL='2'");

			while (rs1.next()) {

				typeID = rs1.getString("TYPE_ID").trim();
				typeName = rs1.getString("TYPE_NAME").trim();
				
				rs =
					db.sqlQueryR(
						"select * from ep_mdic where STAT='1' and TYPE_ID='"
							+ typeID
							+ "' order by ITEM_LEVEL,LIST_ORDER");
