...
function doFinish() { 
	...
	//进行确认提示 
	if(!confirm('点击任务完成后，所有被分配人员将无法填写该任务工时，确认执行操作？')) { 
		return; 
	} 
	document.forms[0].act.value='finish'; 
	document.forms[0].submit(); 
} 
function doQuery() {  
	document.forms[0].act.value = "gmiw";
	document.forms[0].submit(); 
} 
...
<input type=hidden name=act value="gmiw">
...
if (pageResult != null) {%>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<input type="button" value="任务分配" class="Button" onclick="doDistribute()">
			<input type="button" value="任务完成" class="Button" onclick="doFinish()"></td>
		</tr>
	</table>
